...
function ScenariooJasmineReporter(targetDirectory, branchName, branchDescription, buildName, revision) {
    ...
    function suiteDone(suite) {
        ...
        var status = scoEnv.useCases[suite.id];
        var didFail = status.failedScenarios > 0;
        ...
        var logMessage = util.format('useCase :: %s :: %s (%d passed, %d failed, %d skipped)', suite.description, didFail ? 'failed' : 'passed', status.passedScenarios, status.failedScenarios, status.skippedScenarios);
        console.log(logMessage);
        var useCase = {
            name: suite.description,
            description: status.additionalDescription,
            status: didFail ? 'failed' : 'success'
        };
