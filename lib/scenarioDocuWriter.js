...
function writeScenarioXml(currentScenario, absScenarioPath) {
  var data = {
    name: identifierSanitizer.sanitize(currentScenario.scenarioName),
...
var ScenarioDocuWriter = {
  ...
  start: function (branch, build, scenariooTargetDirectory) {
    ...
    this.buildOutputDir = path.join(this.targetDir, utils.encodeFileName(this.branch.name), utils.encodeFileName(build.name));
  ...
  saveBuild: function (build) {
    this.build = build;
    this.build.name = identifierSanitizer.sanitize(this.build.name);
