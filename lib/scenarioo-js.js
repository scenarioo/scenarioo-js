...
function describeUseCase(jasmineDescribeFunction, useCaseName, useCaseDescription, callback) {
  ...
  // prematurely save additional information to our store
...
function describeScenario(jasmineItFunction, scenarioName, scenarioDescription, callback) {
  ...
  // jasmine2 does not return a "spec" object on calling the "it" function.
  // thus, we have to expose the currentScenarioId (see jasmine reporter)
  jasmineItFunction(scenarioName, function () {
    store.updateScenario(jasmine.currentScenarioId, {
