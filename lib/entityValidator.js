...
function validateBuild(build) {
  ...
  if (isValidStatus(build.status)) {
    throw new Error('Build must contain attribute "status" with value "success" or "failed"');
  }
}
...
function validateUseCase(useCase) {
  ...
  if (isValidStatus(useCase.status)) {
    throw new Error('UseCase must contain attribute "status" with value "success" or "failed"');
  }
}
...
function validateScenario(scenario) {
  ...
  if (isValidStatus(scenario.status)) {
    throw new Error('Scenario must contain attribute "status" with value "success" or "failed"');
  }
}
