...
function getCurrentUseCase() {
  if (!state.get('currentUseCase')) {
    state = state.set('currentUseCase', new _immutable2.default.Map());
  }
  return state.get('currentUseCase').toJS();
}

function updateCurrentUseCase(useCase) {
  state = state.update('currentUseCase', new _immutable2.default.Map(), function (originalUseCase) {
...
function getCurrentScenario() {
  if (!state.get('currentScenario')) {
    state = state.set('currentScenario', new _immutable2.default.Map());
  }
  return state.get('currentScenario').toJS();
}

function updateCurrentScenario(scenario) {
  state = state.update('currentScenario', new _immutable2.default.Map(), function (originalScenario) {
